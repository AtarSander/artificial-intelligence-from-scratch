...
def plot_results(experiments_table, g_mean, g_stats, g_stats_std):
    ...
    max_indexes = [np.argmax(g_mean[i:i+batch_size])
                   for i in range(0, len(g_mean), batch_size)]
    timesteps = np.linspace(0, 1000, 400)
    for i in max_indexes:
        legend = f"""\u03BC={experiments_table["Individuals number"][i]}
                    T_max={experiments_table["Iteration number"][i]}
